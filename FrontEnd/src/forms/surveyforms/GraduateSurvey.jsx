...
const GraduateSurvey = ({ department }) => {
    ...
    const handleRatingChange = (index, value) => {
        ...
        // newRatings.forEach((element, index) => {
    ...
    // handle PES form submitting

    const handlePESsubmit = () => {
    ...
    // 12 common POS for all department
    ...
    // 3 specific POS for specific departments
    const depData = [
        {
            Biomedical: [
    ...
    return (
        <div className='pesform'>
            <div>
                <h2>Program Exit Survey</h2>
                <p className='PESnote'>{`As a graduate of ${department} check the ability of yourself to the following statements and rate the performance on a 5-point scale with 5 - Strongly agree, 4- Agree , 3 - Partially agree, 2 - Neutral and 1-Disagree`}</p>
            </div>
            <form>
                <table className='PEsurveytable'>
                    <thead>
                        <tr>
                            <th>s.no</th>
                            <th>QUESTIONNAIRE</th>
                                        ...
                                        [5, 4, 3, 2, 1].map((depRat) => (
                                            <td key={depRat}>
                                                <div className='radio-container'>
                                                    <input
                                                        type="radio"
                                                        name={`specific-rating-${tableData.length + idx + 1}`}
                                                        value={depRat}
                                                        checked={ratings[idx] == depRat}
            ...
            <div className='FormBtndiv'>
                <button className='FormBtn' onClick={handlePESsubmit}>SUBMIT</button>
            </div>
        </div>
    )
}
